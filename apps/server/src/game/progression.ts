...
export function addLootToInventory(characterId: string, item: { id: string; name: string; qty: number }) {
  const runtime = ensure(characterId);
  const existing = runtime.inventory.find((i) => i.itemId === item.id);
  if (existing) existing.quantity += item.qty;
  else runtime.inventory.push({ slot: runtime.inventory.length, itemId: item.id, name: item.name, quantity: item.qty });
}
